...
// FIX: Added `redirectTo` field in the success response so the client can
//      redirect directly to the correct dashboard without relying on
//      middleware chain or router.refresh(), which caused the login loop.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // FIX: Return `redirectTo` so the client can navigate directly.
    //      This prevents the middleware redirect chain that caused the loop.
    const redirectTo = role === "admin" ? "/admin/dashboard" : "/dashboard";

    const response = NextResponse.json<ApiResponse & { redirectTo: string }>({
      status:     "success",
