...
// FIX: Track image load error so the "L" fallback only appears when the logo
//      actually fails to load — previously it was always rendered on top of the image.
...
export default function LoadingScreen({ onComplete, duration = 3200 }: LoadingScreenProps) {
  ...
  // FIX: only show the "L" text fallback when the logo image actually fails
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center px-8 w-full max-w-sm">
        ...
        >
          ...
          {/* FIX: Logo wrapper shows image OR fallback — never both at the same time */}
          <div className="relative h-28 w-28 rounded-3xl overflow-hidden shadow-2xl border-2 border-brand-500/40 bg-white/5 backdrop-blur-sm flex items-center justify-center">
            {!logoError ? (
              ...
              /* "L" is shown ONLY when the image fails to load */
              <span className="text-brand-400 font-black text-4xl font-display select-none">
                L
              </span>
            )}
          </div>
