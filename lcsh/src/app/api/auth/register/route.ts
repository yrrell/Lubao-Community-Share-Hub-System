...
// FIX 1: Notification INSERT now uses the correct column names from the schema
//         (message, type) instead of the non-existent (title, body) columns.
// FIX 2: Admin notification lookup now queries the `admins` table, not the
//         `users` table — admin accounts live in a separate table.
...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.execute({
      ...
      args: [
        username, email, passwordHash, firstName, middleName, lastName,
        birthday, phone, fullAddress, govIdPath, profilePicPath,
      ],
    });

    // ── Notify admins of the new pending registration ──────────────────────
    // FIX: query the `admins` table (not users WHERE role='admin'), and use
    //      the correct notification column names: message + type (no title/body).
    try {
